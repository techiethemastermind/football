  ...
  ngAfterViewInit() {
    ...
    this.getDataService.getJSON('team')
      ...
      data_team => {
        ...
        this.getDataService.getJSON_Param('chart7', 'chart7', this.selected_team)
          .subscribe(
          data => {
            this.data = data;
            if (this.displayType) {
          ...
          if (chk === 0) {
            ...
            if (this.displayTime !== 0) {
              ...
              for (let i = 1; i <= cond; i++) {
                ...
                xd = scale_x(x2) - scale_x(x);
                yd = scale_y(y2) - scale_y(y);

                if (side === 'right') {
                  xd = scale_x(x2) - (660 - scale_x(x));
                  yd = scale_y(y2) - (460 - scale_x(y));
                }
                if (dv.rlt === 0 && this.control.selected.completed === 'active') {
                  line_color = '#ff0';
                  arrow = 'markerArrow_completed';
                  point = { 'id': player_id, 'x': x, 'y': y, 'x2': yd, 'y2': xd, 'line_color': line_color, 'arrow': arrow };
                  data_points.push(point);
                } else if (dv.rlt === 1 && this.control.selected.failed === 'active') {
                  line_color = '#f00';
                  arrow = 'markerArrow_failed';
